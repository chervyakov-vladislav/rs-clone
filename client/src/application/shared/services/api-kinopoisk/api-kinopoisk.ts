class ApiServiceKinopoisk {
  apiKey = '1eed0f50-3f3b-49f1-b171-1547c878ae42';
  ...
  private getTop: string = `${this.filmData}/top?type=TOP_100_POPULAR_FILMS&page=`;
  ...
  public async getTopData(page = 1): Promise<void> {
    const response = await fetch(`${this.getTop}${page}`, {
      headers: {
        Accept: 'application/json',
        'X-Api-Key': this.apiKey,
      },
