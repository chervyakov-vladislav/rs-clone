...
class ApiServiceKinopoisk {
  ...
  private randomIndex = Math.floor(Math.random() * this.apiKeys.length);

  private apiKey = this.apiKeys[this.randomIndex];
  ...
  public async getActorData(id: number): Promise<IActor> {
  ...
  public async searchTopFilms(type = 'TOP_250_BEST_FILMS', page = 1): Promise<ITopData> {
